package com.systemphoenix.edenalpha;

public class CollisionBit {
    public static final short PATHBOUND = 1;
    public static final short PLANTSQUARE = 2;
    public static final short ENEMY = 4;
    public static final short SPAWNPOINT = 8;
    public static final short ENDPOINT = 16;
    public static final short PLANT = 32;
    public static final short ENEMYRANGE = 64;
    public static final short EFFECTIVERANGE = 128;
    public static final short PULSE = 256;
}
